...
dwarp = grad(warpMatrix, {optimize=false})
...
for i=1,nBatches do
   ...
   local bhwdImages
   bhwdImages = data:transpose(2,3):cuda():transpose(3,4):cuda()
   ...
   if opt.warp then
      ...
      for _=1,opt.warpIter do
         -- entropy, prediction, resampledImages = warp(transformParams, params, bhwdImages, target)
         -- grads, entropy, prediction, resampledImages = dwarp(affineMatrices, params, bhwdImages, target)

         -- grads, loss, prediction, resampledImages = dwarp(transformParams, params, bhwdImages, target)
         grads, entropy, prediction, resampledImages = optimfn(params, bhwdImages, target)
         print("==============================")
         -- print(grads:size())
         print(entropy)
         print(prediction:size())
         print(resampledImages:size())
         print("==============================")
