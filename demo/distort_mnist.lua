...
function createDatasetsDistorted(batch, normalize, distort, blur, batchSize)
   local batch = batch or false
   if normalize ~= false then
      local normalize = normalize or true
   end
   if distort ~= false then
      local distort = distort or false
   end
   if blur ~= false then
      local blur = blur or false
   end
   local batchSize = batchSize or 256

   local testFileName, trainFileName, format
   if distort and blur then
      testFileName = 'mnist.t7/test_32x32-distort-and-blur.t7'
      trainFileName = 'mnist.t7/train_32x32-distort-and-blur.t7'
      format = 'binary'
   elseif distort then
      testFileName = 'mnist.t7/test_32x32-distort.t7'
      trainFileName = 'mnist.t7/train_32x32-distort.t7'
      format = 'ascii'
   elseif blur then
      testFileName = 'mnist.t7/test_32x32-blur.t7'
      trainFileName = 'mnist.t7/train_32x32-blur.t7'
      format = 'binary'
   else
      testFileName = 'mnist.t7/test_32x32.t7'
      trainFileName = 'mnist.t7/train_32x32.t7'
      format = 'ascii'
   end

   local train = torch.load(trainFileName, format)
   local test = torch.load(testFileName, format)
